<template>
  <div>
    <!-- Header with actions -->
    <div style="max-width:1200px;margin:0 auto;padding:12px 20px 0 20px;">
      <div style="display:flex;align-items:center;justify-content:flex-end;gap:8px;margin-bottom:8px;">
        ...
        <button class="btn-secondary" @click="handleExport">导出配置</button>
        <button class="btn-secondary" @click="handleImport">导入配置</button>
      </div>
    </div>
      ...
      <div class="sidebar card" style="margin:12px;">
        ...
        <div v-for="prov in builtInProviders" :key="prov.key"
          ...
          <label class="enable-toggle" @click.stop>
            <input type="checkbox" :checked="prov.enabled !== false" @change="handleToggleEnabled(prov.key, ($event.target as HTMLInputElement).checked)" />
            <span class="toggle-label">{{ prov.enabled !== false ? '开' : '关' }}</span>
          </label>
        </div>
...
import type { Config, Provider, Model, BuiltInProvider } from '../types'
...
const customProviders = computed(() => providers.value.filter(p => !p.builtIn))
const selectedProvider = computed(() => {
  return providers.value.find(p => p.key === selectedProviderKey.value) || null
})

const availableBuiltIns = computed(() => {
  const existing = new Set(providers.value.map(p => p.key))
...
function toggleSelectAllFiltered() {
  if (allFilteredSelected.value) {
    ...
    for (const id of filteredIDs) {
      if (!existing.has(id)) selectedModelIDs.value.push(id)
    }
  }
}
...
async function loadData() {
  try {
    ...
    providers.value = (cfg && cfg.providers) ? cfg.providers : []
    apiTypes.value = await a.ListAPITypes()
    allBuiltIns.value = await a.ListBuiltInProviders()
    if (providers.value.length > 0 && !selectedProviderKey.value) {
...
async function handleToggleEnabled(key: string, enabled: boolean) {
  try {
    const a = api()
    await a.SetProviderEnabled(key, enabled)
    await loadData()
    showToast?.(`供应商已${enabled ? '启用' : '禁用'}`, 'success')
  } catch (e: any) {
    showToast?.(e?.message || e, 'error')
...
// ---- Provider drag ----
function onProviderDragStart(e: DragEvent, key: string) { providerDragKey.value = key }
function onProviderDragOver(e: DragEvent, key: string) { providerDragOverKey.value = key }
function onProviderDragEnd() { providerDragKey.value = ''; providerDragOverKey.value = '' }
...
async function onProviderDrop(e: DragEvent, targetKey: string) {
  ...
  const srcIdx = provs.findIndex(p => p.key === srcKey)
  const tgtIdx = provs.findIndex(p => p.key === targetKey)
  if (srcIdx < 0 || tgtIdx < 0) return
  const [moved] = provs.splice(srcIdx, 1)
  provs.splice(tgtIdx, 0, moved)
  const orderedKeys = provs.map(p => p.key)
  try {
    const a = api()
    await a.ReorderProviders(orderedKeys)
    await loadData()
...
// ---- Model drag ----
function onModelDragStart(e: DragEvent, id: string) { modelDragID.value = id }
function onModelDragOver(e: DragEvent, id: string) { modelDragOverID.value = id }
function onModelDragEnd() { modelDragID.value = ''; modelDragOverID.value = '' }
...
async function onModelDrop(e: DragEvent, targetID: string) {
  ...
  try {
    const a = api()
    await a.ReorderModels(selectedProviderKey.value, orderedIDs)
    await loadData()
...
// ---- Built-in add ----
function resetAddBuiltIn() { addBuiltInKey.value = ''; addBuiltInAPIKey.value = ''; addBuiltInBaseURL.value = ''; addBuiltInError.value = ''; showBuiltInKey.value = false }

async function handleAddBuiltIn() {
  if (!addBuiltInKey.value) return
  try {
    const a = api()
    await a.AddBuiltInProvider(addBuiltInKey.value, addBuiltInAPIKey.value, addBuiltInBaseURL.value)
...
// ---- Custom add ----
function resetAddCustom() { addCustomKey.value = ''; addCustomBaseURL.value = ''; addCustomAPIType.value = ''; addCustomAPIKey.value = ''; addCustomError.value = ''; showCustomKey.value = false }

async function handleAddCustom() {
  try {
    const a = api()
    await a.AddCustomProvider(addCustomKey.value, addCustomBaseURL.value, addCustomAPIType.value, addCustomAPIKey.value)
...
async function handleSaveProvider() {
  ...
  try {
    ...
    await a.UpdateProvider(updated)
    provError.value = ''
    await loadData()
...
async function handleRemoveProvider() {
  ...
  try {
    const a = api()
    await a.RemoveProvider(selectedProvider.value.key)
...
async function handleTestConnectivity() {
  ...
  try {
    const a = api()
    const msg = await a.TestProviderConnectivity(selectedProvider.value.key)
...
async function handleSaveModel() {
  ...
  try {
    ...
    if (editingModel.value) {
      await a.UpdateModel(selectedProviderKey.value, m)
    } else {
      await a.AddModel(selectedProviderKey.value, m)
    }
    closeModelForm()
    await loadData()
...
async function handleDeleteModel() {
  ...
  try {
    const a = api()
    await a.RemoveModel(selectedProvider.value.key, id)
    confirmDeleteModel.value = ''
    await loadData()
    showToast?.('模型已删除', 'success')
  } catch (e: any) { showToast?.(e?.message || e, 'error') }
}
...
async function handleBatchDelete() {
  ...
  try {
    const a = api()
    const removed = await a.RemoveModels(selectedProvider.value.key, ids)
...
function resetBatchImport() { batchImportJSON.value = ''; batchImportError.value = ''; batchImportResult.value = '' }
...
async function handleBatchImportSubmit() {
  ...
  let parsed: any
  try { parsed = JSON.parse(batchImportJSON.value) } catch { batchImportError.value = 'JSON 格式错误'; return }
  if (!Array.isArray(parsed)) { batchImportError.value = 'JSON 格式错误：应为数组'; return }
  const models: Model[] = []
  for (const item of parsed) {
    if (!item.id || typeof item.id !== 'string') { batchImportError.value = 'JSON 格式错误：每项必须包含 id 字段'; return }
    models.push({
      id: item.id, name: item.name || item.id,
      reasoning: !!item.reasoning, inputText: item.inputText !== undefined ? !!item.inputText : true,
      inputImage: !!item.inputImage, contextWindow: typeof item.contextWindow === 'number' ? item.contextWindow : 256000,
  ...
  try {
    const a = api()
    const added = await a.ImportProviderModels(selectedProviderKey.value, models)
    const skipped = models.length - added
    batchImportResult.value = added > 0 ? `成功导入 ${added} 个模型，跳过 ${skipped} 个已存在模型` : '无新增模型（所有模型均已存在）'
    batchImportResultType.value = added > 0 ? 'toast-success-inline' : 'toast-error-inline'
    await loadData()
  } catch (e: any) { batchImportError.value = e?.message || e }
}
...
async function handleFetchModels() {
  ...
  try {
    const a = api()
    const result = await a.FetchProviderModels(prov.key)
...
function handleFetchSelectAll() {
  fetchSelectedIds.value = fetchSelectAll.value ? fetchedModels.value.map(m => m.id) : []
}

async function handleImportModels() {
  if (fetchSelectedIds.value.length === 0) { showToast?.('请至少选择一个模型', 'error'); return }
  try {
    const selectedModels = fetchedModels.value.filter(m => fetchSelectedIds.value.includes(m.id))
    const a = api()
    const count = await a.ImportProviderModels(selectedProviderKey.value, selectedModels)
    ...
    if (count > 0) {
      ...
      showToast?.(skipped > 0 ? `已导入 ${count} 个模型，${skipped} 个因重复跳过` : `已导入 ${count} 个模型`, 'success')
    } else {
